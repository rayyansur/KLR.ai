...
def process_query(text_query: str, image_path: str) -> dict:
    ...
    detections = yolo_results.get("Objects", [])  # YOLO returns "Objects" key
...
def process_auto_detect(image_path: str) -> dict:
    ...
    # Skip Azure LLM for now - just return basic detection info
    if detections:
        object_names = [d.get('class', 'object') for d in detections]
        response_text = f"Detected {len(detections)} object(s): {', '.join(object_names[:5])}"
    else:
        response_text = "No objects detected in the image."
